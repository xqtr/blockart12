...
Procedure NESExtract(Var Img:TConsoleImageRec);
...
Begin
  for o:=1 to cheight do begin
    WriteXY(cx,cy+o,7,strrep(cch,cwidth));
  end;
end;
...
Begin
  Case Color Of
    ...
    3 : Cl := 0;
  End;
  WriteXY(offx+x,offy+y,cl,Chr(219));
End;

Function NESTile(loc,x,y: Integer):Byte;
Var 
  r,c,b : Integer;
...
Begin	
  ...
  r := 0;
    ...
    For c := 0 To 7 Do Begin
      If BitCheck(c,1,Data[r]) And BitCheck(c,1,Data[r+8]) Then Color :=3 else
        If BitCheck(c,1,Data[r]) And Not BitCheck(c,1,Data[r+8]) Then Color :=1 else
          If Not BitCheck(c,1,Data[r]) And BitCheck(c,1,Data[r+8]) Then Color :=2 else
            Color := 0;
...
Begin
  For r := 0 to 2 Do Begin
    For c := 0 To 11 Do Begin
      NESTile(loc+((c+(r*Width))*(8*bpp)), c*8, r*8);
    End;
  End;
...
Begin
  ...
  GetDir(0,DataPath);
  RomFile := GetUploadFilename(' ROM File ',DataPath,'*.*');

  If RomFile = '' Then Begin
    ShowMsgBox(0,'No ROM Specified');
    Exit;
  End;
...
Repeat
  DrawFile(Loc);
  //GBTile(Loc,40,12);
  WriteXY(1,25,8,Int2Str(Loc)+ ' / '+Int2Str(EOFLoc));
  //WriteXYPipe(17,25,7,62,'|15W|08/|15E |07Scroll Line |15S|08/|15D |07Scroll Pos. |15X|08/|15C |07Scroll Page |15ENTER |07Select |15ESC |07Quit');
  WriteXYPipe(17,25,7,'|15W|08/|15E |07Scroll Line |15S|08/|15D |07Scr.Pos. |15X|08/|15C |07Scr.Page |15ENTER:|07Select |15ESC:|07Quit');
  Case ReadKey Of
    #00 : Case ReadKey Of
            #45: Done := True;
