...
Function ManageBlock(Var Layer:TConsoleImageRec; ScAttr:Byte; CharsetNo:Byte):Boolean;
...
Procedure DrawBlock(Image: TConsoleImageRec; Str: String; Pick:Boolean; Under:Boolean);
Var
  x,y,a,b,t,z   : Byte;
...
Begin
  ...
  x := WhereX;
  y := WhereY;
  RestoreScreen(BufImg);
  WriteXYPipe(1,25,7,StrpadR(str,80+(Length(str)-StrMCILen(str)),' '));
  GotoXY(x,y);
...
Function SaveBlockAsTDFChar(Var Image: TConsoleImageRec):Boolean;
Type
  TWB = record
    b1, b2: byte;
  end;
...
Begin 
    ...
    End;
  End;
  s:=xtdf.AvailableChars;
  Writexy(16,20,3,Copy(s,1,47));
  Writexy(16,21,3,Copy(s,48,47));
  c:=GetTDFChar;
  If c=0 Then Exit;
  Ch := Chr(c);
  If Pos(Ch,S)>0 Then Begin
    If ShowMsgBox(1,'Character exists. Proceed?')=False Then Exit;
  End;
  
  Assign(fp,FontFile);
  {$I-}Reset(fp,1);{$I+}
  If IOResult<>0 Then Exit;
  BlockRead(fp,tdf,sizeof(tdf));
  BlockRead(fp,tfh,sizeof(tfh));
  
  //Go to EOF
  Seek(fp,filesize(fp));
  
  //Calculate Char Offset and write to Header
  offset:=filesize(fp)-233;
  readkey;
  tfh.CharAddr[(c-33)*2]:=TWB(offset).b1;
  tfh.CharAddr[((c-33)*2)+1]:=TWB(offset).b2;
  
  //Calculate Width and Write to File
  q:=b-a+1;
  blockwrite(fp,q,1);
  //Calculate Height and Write to File
  q:=z-t+1;
  blockwrite(fp,q,1);
  
  //Write Char DATA
  If tfh.FontType=2 then begin  //Color Font
    For y := t To z Do Begin
      For x := a To b Do Begin
        blockwrite(fp,Image.Data[y][x].UnicodeChar,1);
        blockwrite(fp,Image.Data[y][x].Attributes,1);
      End;
        q:=$0d;
        blockwrite(fp,q,1);
    End;
    ...
    blockwrite(fp,q,1);
  End;
  If tfh.FontType=1 then begin  //block font
    For y := t To z Do Begin
      For x := a To b Do 
        blockwrite(fp,Image.Data[y][x].UnicodeChar,1);
      q:=$0d;
    ...
    blockwrite(fp,q,1);
  End;
  //Go to Start of Header and write new header
  seek(fp,20);
  blockwrite(fp,tfh,sizeof(tfh));
  //Go to Blocksize field and write the new BlockSize
  seek(fp,43);
  w:=filesize(fp)-233;
  blockwrite(fp,w,2);
  Close(fp);
...
Begin
  ...
  End;
  For x := a To b Do Begin
    Image.Data[t][x].UnicodeChar:=CharSet[d];
  ...
  End;
  For y:= t To z Do Begin
    Image.Data[y][a].UnicodeChar:=CharSet[d];
  ...
  End;
  For y:= t To z Do Begin
    Image.Data[y][b].UnicodeChar:=CharSet[d];
  ...
  Case CharsetId Of
    ...
    5 : d:=4;
  End;
  Image.Data[t][a].UnicodeChar:=CharSet[d];
  ...
  Case CharsetId Of
    ...
    5 : d:=4;
  End;
  Image.Data[t][b].UnicodeChar:=CharSet[d];
  ...
  Case CharsetId Of
    ...
    5 : d:=4;
  End;
  Image.Data[z][a].UnicodeChar:=CharSet[d];
  ...
  Case CharsetId Of
    ...
    5 : d:=4;
  End;
  Image.Data[z][b].UnicodeChar:=CharSet[d];
  Image.Data[z][b].Attributes := Attr;
End;

Procedure FillBlock(Var Image: TConsoleImageRec);
Var
  x,y   : Byte;
  fg,bg : Byte;
  r     : String;
...
Procedure MoveBlock(Var Image: TConsoleImageRec);
Var
...
Begin
  ...
  Repeat
  DrawBlock(Image,'|08[|15O|08]|07ver |08[|15U|08]|07nder |08[|15ESC|08] |07Cancel |08[|15ENTER|08] |07Confirm',True,Under);
    Case ReadKey Of
      #00 : Begin
              
              Case Readkey Of
                KeyCursorLeft : If a > 1 Then Begin
                            a := a - 1;
                            b := b - 1;
                            Block.X1 := a;
                            Block.X2 := b;
                          End;
                KeyCursorRight: If a + Width < 80 Then Begin
                            a := a + 1;
                            b := b + 1;
                            Block.X1 := a;
                            Block.X2 := b;
                          End;
                KeyCursorUp   : If t > 1 Then Begin
                            t := t - 1;
                            z := z - 1;
                            Block.Y1 := t;
                            Block.Y2 := z;
                          End;
                KeyCursorDown : If t < 24 - Height Then Begin
                            t := t + 1;
                            z := z + 1;
                            Block.Y1 := t;
                            Block.Y2 := z;
...
Procedure CopyBlock(Var Image: TConsoleImageRec);
Var
  Img,Img2  : TConsoleImageRec;
  OBlock    : TBlock;
  Width     : Byte;
  ...
  Function LoadSelection(Var Orig: TConsoleImageRec; Filename: String):Boolean;
  Var
  ...
  Begin
    Clrscr;
    //LoadANSIFile(Screen,Filename);   TO FIX!!!
    LoadAnsi2Image(filename,buf,r);
    restorescreen(buf);
    Repeat
      DrawBlock(Buf,'|08[|15ESC|08] |07Cancel/Stop |08[|15ENTER|08] |07Confirm',False,Under);
      Case ReadKey Of
        #00 : Begin
                Case Readkey Of
                  KeyCursorLeft : If a > 1 Then Begin
  ...
  Procedure SaveSelection(Image: TConsoleImageRec);
  Var 
    buf : TConsoleImageRec;
    Filename: String;
    OutFile : Text;
  ...
  Begin
    ...
    GetDir(0,xferpath);
    Filename := GetSaveFileName(' Save Selection ','selection.ans',xferpath,'*.ans');
      ...
      For Count1 := Block.Y1 to Block.Y2 Do Begin
        For Count2 := Block.X1 to Block.X2 Do Begin
          If OldAt <> Image.Data[Count1][Count2].Attributes then Begin
            ...
            System.Write(Outfile,Ansi_Color(FG,Attr));
            System.Write(Outfile,Ansi_Color(BG,Attr));
...
Begin
  ...
  Repeat
  DrawBlock(Image,'|08[|15O|08]|07ver |08[|15U|08]|07nder |08[|15C|08]|07lone |08[|15P|08]|07aste |08[|15S|08]|07ave |08[|15F|08]|07ont Char. |08[|15ESC|08] |07Cancel/Stop',True,Under);
    Case ReadKey Of
      #00 : Begin
              Case Readkey Of
                KeyCursorLeft : If a > 1 Then Begin
  ...
  'c','C' : Begin
            ...
            End;
  {'l','L' : Begin
              S := GetUploadFileName('Load File',xFerPath,'*.*');
              If S = '' Then Break;
              If LoadSelection(Image,S) Then Break;
            End;}
  's','S' : Begin
              SaveSelection(Image);
            End;
  'f','F' : Begin
              SaveBlockAsTDFChar(Image);
              xTDF.Init(FontFIle);
            End;
    End;
  Until False;
...
Begin
  ...
  Drawmode := Draw_Normal;
  Repeat
  DrawBlock(Image,'|07[|15M|07]ove |07[|15C|07]opy |07[|15E|07]rase |07[|15F|07]ill |07[|15B|07]ox |07[|15X|07]Flip |07[|15Y|07]Flip |07[|15L|07]ine Ci|07[|15R|07]cle |07[|153|07]D |07[|15ESC|07]',False,False);
    Case ReadKey Of
      #00: Begin
              Case Readkey Of
                KeyCursorUp   : KeyUp;
                KeyCursorDown : KeyDown;
                KeyCursorLeft : KeyLeft;
                KeyCursorRight: KeyRight;
                KeyHome : CursorHome;
                KeyEnd  : CursorEnd;
                KeyPGUP : CursorPGUP;
                KeyPGDN : CursorPGDN;
  ...
  'l','L': Begin
              s:='|07[|15T|07]op - Bottom  |07[|15B|07]ottom - Top |07[|15ESC|07] Cancel';
              WriteXYPipe(1,25,7,StrpadR(s,80+(Length(s)-StrMCILen(s)),' '));
              ch := Chr(GetChar);
              Repeat 
              Case Readkey Of
                #27    : Break;
